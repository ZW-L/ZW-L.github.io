...
属性|描述|取值
-|-|-
autoplay|音频自动开始播放，无需等待整个音频文件完成下载|布尔值，默认 false。
controls|浏览器提供控件允许用户控制音频播放(音量, 搜索, 暂停/继续播放)|布尔值，默认 false。
loop|指定音频循环播放|布尔值，默认 false。
muted|指示音频是否最初会静音|布尔值，默认 false。
preload|向浏览器提示作者认为将带来最佳用户体验的内容|`none`: 不预加载音频<br>`metadata`: 仅获取音频元数据<br>`auto`: 下载整个音频文件<br>`空串`: 同 `auto`
src|嵌入的音频的URL，受 `HTTP` 访问控制的约束。<br>可以改为使用 `<source>` 音频块中的元素来指​​定要嵌入的音频|URL
currentTime|读取/设置音频的当前播放位置，以秒为单位|双精度浮点值
duration|读取音频的总长度，以秒为单位|双精度浮点值(只读)
ended|获取音频是否结束|布尔值(只读)
paused|获取音频是否暂停|布尔值(只读)
networkState|获取音频的网络状态|只读，对应值:<br>`0`: 尚未初始化<br>`1`: 已经选取资源，但未使用网络<br>`2`: 正在下载数据<br>`3`: 未找到资源
readyState|获取音频当前状态|只读，对应值:<br>`0`: durationchange 事件触发之前<br>`1`: 音频元数据已获取<br>`2`: 当前播放位置的数据是可用的，但下一帧不一定可用<br>`3`: 当前及至少下一帧的数据是可用的<br>`4`: 可用数据足以开始播放
buffered|获取已缓冲的时间区间|只读，属性:<br>`buffered.length`: 区间个数<br>`buffered.start(index)`: 指定索引区间的起始时间<br>`buffered.end(index)`: 指定索引区间的结束时间
played|获取已播放的时间区间|只读，属性:<br>`played.length`: 区间个数<br>`played.start(index)`: 指定索引区间的起始时间<br>`played.end(index)`: 指定索引区间的结束时间
seekable|获取可寻址的时间区间|只读，属性:<br>`seekable.length`: 区间个数<br>`seekable.start(index)`: 指定索引区间的起始时间<br>`seekable.end(index)`: 指定索引区间的结束时间
seeking|获取用户是否在寻址，在 `seeking` 事件中值为 `true`|布尔值(只读)
volume|获取/设置音量值|浮点值，0~1。
playbackRate|获取/设置播放倍速|浮点值，默认 1。
crossorigin|指示是否使用 `CORS` 来获取相关的音频文件|`anonymous`: 发送没有凭证的跨域请求<br>`use-credentials`: 发送带有凭据的跨域请求
disableRemotePlayback|用于禁用使用有线（HDMI，DVI等）和无线技术（Miracast，Chromecast，DLNA，AirPlay等）<br>连接的设备中的远程播放功能|布尔值
