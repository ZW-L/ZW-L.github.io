...
+ `TCP/IP`(Transmission Control Protocol/Internet Protocol, 传输控制协议/网络互联协议)是一组用于实现网络互联的通信协议，是 Internet 最基本的协议和互联网络的基础，其：
  + 定义了在互联网中如何传输数据，如文件传送、收发电子邮件、远程登录等
  + 制定了在出错时必须遵守的规则
...
+ 不依赖于特定的网络传输硬件，因此能集成各种各样的网络
+ 不依赖于任何特定的计算机硬件或操作系统
+ 基于 TCP/IP 的网络使用统一的寻址系统
...
+ **数据报的路由转发**：网络上的每个中间节点(路由器)都根据 IP 数据包中接收方的目标 IP 地址来确定是`本网传送`还是`跨网传送`
